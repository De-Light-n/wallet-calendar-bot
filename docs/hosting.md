...
> **Один процес** — `uvicorn app.main:app` запускає **все**: HTTP API, Slack
> webhook, Discord gateway, Telegram (webhook на проді або long-polling
> локально). Окремих воркерів типу `bot.py` більше немає.
>
> Telegram автоматично перемикається між режимами:
> - **Webhook** — якщо у `.env` є валідний публічний `WEBHOOK_URL` (HTTPS).
> - **Long polling** як background task — якщо `WEBHOOK_URL` порожній або
>   локальний (наприклад на dev-машині).
...
4. **Telegram webhook конфліктує з polling** → `getWebhookInfo` показує URL, але app намагається polling (бо `WEBHOOK_URL` десь став невалідним). Запуск `app/main.py` дропає старий webhook перед стартом polling, тому це автоматично розрулюється на дев. На проді — переконайся що `WEBHOOK_URL` валідний (HTTPS, публічний), щоб webhook-режим запустився.
