...
export function ConnectTelegram() {
  ...
  const mutation = useMutation({
    mutationFn: () => api.post<LinkCodeResponse>('/api/link-codes'),
  ...
  return (
    <section className="card">
      <h2>
        Telegram {tgConnected ? <span className="badge ok">підключено</span> : <span className="badge muted">не підключено</span>}
      </h2>
      ...
      {tgConnected ? (
        ...
        <>
          <p className="muted">
            Щоб підключити Telegram, згенеруй одноразовий код і введи його в
            боті командою <code>/link</code>.
          </p>
          ...
          {code && (
            <div className="code-block">
              ...
              <p className="muted small">Код діє 10 хвилин. Після прив'язки оновіть сторінку.</p>
              <button className="ghost-btn" onClick={() => { setCode(null); refresh() }}>
                Готово
              </button>
            </div>
          )}
